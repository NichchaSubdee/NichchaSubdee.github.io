...
navbar:
  ...
  - { title: "GitHub", href: "https://github.com/NichchaSubdee", target: "_blank" }
  - { title: "LinkedIn", href: "https://www.linkedin.com/in/nichcha-subdee-b94247108/", target: "_blank" }
...
```{r plot1, message=FALSE, include=FALSE}
scatter_df |> 
  mutate(
    text_label = paste(
      "Date:", date,
...
```{r plot2, message=FALSE, include = FALSE}
box_df |>
  plot_ly(
    y = ~snow, color = ~month,
    type = "box"
...
```{r plot3, message=FALSE, include=FALSE}
bar_df |>
  plot_ly(
    x = ~month, y = ~n_days,
